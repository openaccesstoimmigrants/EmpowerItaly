import React from 'react';

function Hypothesis() {
  return (
    <section id='hypothesis' className='py-24 mx-auto max-w-2xl'>
        <div className='flex flex-col text-center w-full mb-12'>
          <h2 class="text-xs text-indigo-500 tracking-widest font-medium mb-3">OUR HYPOTHESIS</h2>
          <h1 class="text-3xl font-bold text-gray-900">Are immigrants in Italy more likely to be unemployed compared to the native population?</h1>
        </div>
        <p className='lg:w-2/3 mx-auto leading-relaxed text-center text-base'>
              This hypothesis question addresses the potential difference in unemployment rates between immigrants and native Italians. It allows to investigate the labor market dynamics and assess whether there might be disparities in employment opportunities for immigrants.
        </p>
    </section>
  )
}

export default Hypothesis